...
If implementing multiple models at each GPUs(e.g. ensemble) is required, each GPU can be allocated to models(graph). Additionally, if each graph shares lots of common parts such as input, output, dropout, and cost, declare graph with "scope" allow us to access each component of graph more efficiently at training parts. In order to show how to access shared component name across different graph, I only changed the size of node at the last layer of each graph and named components same.
...
Once the size of necessary model exceeds that of GPU memory, model can be divided into multiple jobs where each of them are executed by different GPUs.
