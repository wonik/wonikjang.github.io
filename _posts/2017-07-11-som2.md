...
If differently labeled images located at the same node in SOM lattice, the previous visualization will only display one of them.  At least for the good and bad (+/-) images of a product, it should be possible to display them on left and right positions of the node point with different colors as follows.
...
To resolve this problem, I generate each folder( folder name == node order) and save each image into the corresponding folder. In addition, we can create CSV file with folder name, the number of +/- images in a folder to confirm what kind of images are clustered in each node.
