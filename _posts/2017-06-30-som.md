...
Suppose your mission is to cluster colors, images, or text. Unsupervised learning, which means that no label information is provided, can be applied in such project. Specifically for image clustering, the most widely used algorithm are Self-Organizing-MAP(SOM) and Adaptive Response Theory(ART). 

In terms of SOM, it's an unsupervised learning method to cluster inputs by projecting it into a lattice of neurons (locations of neurons), each of which is fully connected to the input layer, but not connected between neurons. Although there is no concept of location of neurons in Artificial Neural Network(ANN), SOM presumes that the closer neurons and inputs the similar they are. Such closest neuron is denoted as Best Matching Unit(BMU), and calculated based on the Euclidean distance from inputs. The below figure demonstrates a small network of 5*4 neurons. For each iteration, after BMU is determined, SOM calculates the weights of other units within BMU's neighborhood. As iteration number increases, the length of neighborhood radius and learning rate decrease.
...
Each of the plotted MNIST test digits is well fitted into the target locations! Although digit 3 and 5 are not completely splitted, it seems to be resolved if the number of training iteration increases.
