...
The 'Map' of SOM indicates the locations of neurons, which is not a prevalent case in Artificial Neural Network(ANN). Based on this 'loactions', SOM presumes that the closer neurons and inputs are the similar properties they have. Such closest neuron is denoted as Best Matching Unit(BMU), and calculated based on the Euclidean distance from inputs. The below figure demonstrates a small network of 5*4 neurons. For each iteration, after BMU is determined, SOM calculates the weights of other units within BMU's neighborhood. As iteration number increases, the length of neighborhood radius and learning rate decrease.
